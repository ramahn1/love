...
// reset position with touch
            if(mouse.x){this.ref.x=mouse.x;
                ...
                mouse.y=null;
                this.si=Math.min(0.05*this.ref.x,0.05*(W-this.ref.x),0.05*this.ref.y,0.05*(H-this.ref.y));
                ctx.fillStyle = "rgba(230, 224, 228, 0.95)";
...
const animate = () => {
    ...
    if(stat){
    u.writeT("HAII SAYANG",0.5*W,0.85*H);
u.writeT("MET BOBO YAA",0.5*W,0.9*H);
    u.writeT("LUVV BABAYYY",0.5*W,0.95*H);}
...
const init = () => {
    ...
    ctx.fillStyle = "rgba(238, 236, 243, 0.99)";
    ctx.fillRect(0, 0, W, H);
    eventsListener();
    requestAnimationFrame(animate);
};
